...
import { getAllSales, deleteSale } from '../../redux/slice/saleSlice'
import { toNumber, formatDate, sortData } from '../../components/common/formatter'
import SaleAddEditModel from './SaleAddEditModel'
...
function Sale() {
  ...
  const { result: orders, isLoading } = useSelector((state) => state.sales)
  const [visible, setVisible] = useState(false)
  const [editData, setEditData] = useState(null)
  const { addToast } = useToast()

  const [sortConfig, setSortConfig] = useState({ key: 'soNumber', direction: 'desc' })
  ...
  const stats = useMemo(() => {
    ...
    return orders.reduce(
      (acc, curr) => ({
        discount: acc.discount + Number(curr.totalDiscount || curr.TotalDiscount || 0),
        tax: acc.tax + Number(curr.totalTax || curr.TotalTax || 0),
        total: acc.total + Number(curr.totalAmount || curr.TotalAmount || 0),
      }),
  ...
  const getStatusBadge = (status) => {
    ...
    if (s === 'ordered' || s === 'delivered') return 'success'
    if (s === 'pending' || s === 'partial' || s === 'partially delivered') return 'warning'
  ...
  return (
    <CRow>
      ...
      <CCol xs={12}>
        <CCard className="shadow-sm border-0 mt-2">
          <CCardBody>
            ...
            {isLoading ? (
              <AppLoader message="Fetching Sales Data..." />
            ) : (
                    ...
                    sortedOrders.map((order) => (
                      <CTableRow key={order.id || order.ID} className="align-middle">
                        ...
                        <CTableDataCell className="fw-bold text-dark">
                          {toNumber(order.totalAmount || order.TotalAmount || 0, 2)}
                        </CTableDataCell>
                        <CTableDataCell>
                          <CBadge
                            color={getStatusBadge(order.status || order.Status)}
                            shape="rounded-pill"
      ...
      <SaleAddEditModel visible={visible} setVisible={setVisible} editData={editData} />
    </CRow>
  )
}
